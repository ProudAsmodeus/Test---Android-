...
for apk in stock_root.rglob("*.apk"):
    low = str(apk).lower()
    if "camera" in low and ("samsung" in low or "sec" in low or "com.sec.android.app.camera" in low):
        apk_candidates.append(apk)

selected_apk = None
if apk_candidates:
    selected_apk = max(apk_candidates, key=score_apk)
    target_apk = prebuilt_dir / "SamsungCamera.apk"
    shutil.copy2(selected_apk, target_apk)
    print(f"Selected stock camera APK: {selected_apk}")
    print(f"Copied to: {target_apk}")
else:
    print("No Samsung camera APK candidate found in stock dump.")
